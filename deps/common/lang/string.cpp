...
{
  int num = v*1000;
  //by haijun, 用于修复四舍五入的问题
  if(num%10 == 5) {  //这种情况下，表示小数是正数，小数点后第三位是5，舍入时会出现问题
    v += 0.002;   //让小数点后第三位超过5，后面的代码就可以正常的执行四舍五入的逻辑了
  }
  if(num%10 == -5) {  //这种情况下，表示小数是负数，小数点后第三位是5，舍入时会出现问题
    v -= 0.002;   //让小数点后第三位超过5，后面的代码就可以正常的执行四舍五入的逻辑了
  }
  char buf[256];
