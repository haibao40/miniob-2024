      ...
      std::vector<ViewAttrInfoSqlNode> child_attributes, span<const ViewAttrInfoSqlNode> attributes)
{
  ...
  // 创建文件
  if ((rc = view_meta_.init(view_id, name, conditions, child_conditions, child_attributes, attributes)) != RC::SUCCESS) {
...
bool View::insert_capacity(std::vector<std::string> field_names) { return view_meta_.insert_capacity(field_names); }
